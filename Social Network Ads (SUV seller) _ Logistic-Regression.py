...
classifier = LogisticRegression(C= 0.1, max_iter= 100, multi_class= 'ovr', penalty= 'l1', solver= 'liblinear',random_state = 0)
...
X1, X2 = np.meshgrid(np.arange(start = X_set[:, 0].min() - 10, stop = X_set[:, 0].max() + 10, step = 0.0625),
...
output= [['Logistic Regression', 'Yes', accuracies.mean()*100, accuracies.std()*100, accuracy_score(y_test, y_pred)]]
